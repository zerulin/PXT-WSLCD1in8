...
//Block enumeration values (DOT_PIXEL / LINE_STYLE / DRAW_FILL) arrive from
//TypeScript as plain numbers.  LCD_BACKGROUND_COLOR is the colour the dotted
//line style uses for the gaps, same value as the V1 release.
#define LCD_BACKGROUND_COLOR    0x0001
#define LCD_LINE_SOLID          0
#define LCD_LINE_DOTTED         1
#define LCD_DRAW_EMPTY          0
#define LCD_DRAW_FULL           1
...
namespace lcdbus {
    ...
    void pixel(uint32_t Addr, uint16_t Color);     //one frame buffer pixel (runs are merged)
    void sync();                                   //close the frame buffer run: call before
                                                   //using the LCD or the SRAM directly
}
...
private:
    ...
    void LCD_Point(int Xpoint, int Ypoint, int Dot_Pixel, int Color);
...
public:
    ...
    void LCD_DrawRectangle(int Xstart, int Ystart, int Xend, int Yend, int Color, int Filled, int Dot_Pixel);
    void LCD_DrawCircle(int X_Center, int Y_Center, int Radius, int Color, int Draw_Fill, int Dot_Pixel);
