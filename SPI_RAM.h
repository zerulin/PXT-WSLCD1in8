...
public:
    ...
    //single 16 bit pixel in one SPI transaction (the fast path used by
    //LCD_SetPoint: the byte mode address does not auto increment, so command,
    //address and both data bytes belong to the same chip select phase)
    void SPIRAM_WR_Word(uint32_t Addr, uint16_t Data);
