...
*                   The blocks are unchanged, but all hardware access (LCD and
*                   the SPI SRAM frame buffer) now runs in the native driver
*                   (LCD1in8.cpp + LCD_Driver.cpp + SPI_RAM.cpp) like the V1
*                   release did.  Pushing every byte through pins.spiWrite()
*                   from TypeScript was what made the graphics refresh slow.
*                   LCD/RAM geometry lives in LCD_Driver.h, the font table in
*                   Font12.h.
*
******************************************************************************/
